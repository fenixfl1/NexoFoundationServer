...
$dumpArgs = @("-h", $srcHost, "-p", $srcPort, "-U", $srcUser, "-d", $srcDb, "--no-owner", "--no-privileges")

$proc = Start-Process -FilePath $pgDump `
  -ArgumentList $dumpArgs `
  -NoNewWindow `
  -RedirectStandardOutput "$env:TEMP\nexo_local_sync_${timestamp}.sql" `
  -Wait `
  -PassThru
...
& $psql $dstUri -f "$env:TEMP\nexo_local_sync_${timestamp}.sql"
Remove-Item Env:PGPASSWORD -ErrorAction SilentlyContinue
Remove-Item "$env:TEMP\nexo_local_sync_${timestamp}.sql" -Force
